...
import { Activity, AlertTriangle, BarChart3, Bell, BellOff, ChevronRight, CircleDollarSign, Database, ExternalLink, Gauge, History, Layers3, LoaderCircle, Newspaper, Play, RefreshCw, Save, ShieldCheck, Trash2, Wifi, WifiOff } from 'lucide-react'
import { api, normalizeAdvice } from './api'
import type { AdviceAction, BacktestResult, Candle, MarketRegime, MarketSnapshot, NewsResponse, Settings, SignalAdvice, TechnicalSummary } from './types'
...
function AdviceCard({ advice, loading }: { advice: SignalAdvice | null; loading: boolean }) {
  ...
  return <section className={`panel advice-card ${tone}`}>
    ...
    {loading && !advice ? <div className="big-empty"><LoaderCircle className="spin"/>正在计算已收盘 K 线…</div> : !advice ? <div className="big-empty"><AlertTriangle/>暂无有效建议，等待后端分析</div> : <>
      ...
      <div className="score-breakdown"><Metric label="技术面" value={`${signed(advice.technicalScore,0)} / 85`} tone={advice.technicalScore>0?'up':advice.technicalScore<0?'down':''}/><span>+</span><Metric label="消息面" value={`${signed(advice.newsScore,0)} / 15`} tone={advice.newsScore>0?'up':advice.newsScore<0?'down':''}/><span>=</span><Metric label="综合方向" value={`${signed(advice.directionScore,0)} / 100`} tone={advice.directionScore>0?'up':advice.directionScore<0?'down':''}/></div>
...
function TechnicalPanel({ data, error }: { data: TechnicalSummary|null; error: string }) {
  return <section className="panel wide-panel technical-panel"><div className="section-title"><span><Layers3 size={17}/>技术共振</span><span className={`module-status ${data?.status??'unavailable'}`}>{data?.status==='fresh'?'数据新鲜':data?.status==='partial'?'部分数据':'暂不可用'} · {time(data?.asOf)}</span></div>
    {error&&!data?<div className="module-empty"><AlertTriangle/>{error}</div>:!data?<div className="module-empty"><LoaderCircle className="spin"/>正在汇总多周期技术指标…</div>:<>
      <div className="technical-groups">{data.groups.slice(0,5).map(g=><article className="technical-group" key={g.key}><div className="tech-group-head"><strong>{g.label}</strong><div><span className={ratingTone(g.rating1h)}>1H {g.rating1h}</span><span className={ratingTone(g.rating4h)}>4H {g.rating4h}</span></div></div><p>{g.summary||'等待更多已收盘 K 线确认'}</p><div className="tech-metrics">{g.metrics.slice(0,4).map((m,i)=><span key={`${m.label}-${i}`}><small>{m.label}</small><b className={m.tone==='positive'?'up':m.tone==='negative'?'down':''}>{m.value}</b></span>)}</div></article>)}</div>
      <div className="technical-levels"><Metric label="VWAP" value={money(data.vwap)}/><Metric label="成交量 POC" value={money(data.poc)} hint="成交最密集价"/><Metric label="VAH / VAL" value={`${money(data.vah)} / ${money(data.val)}`} hint="价值区上下沿"/><Metric label="支撑" value={data.supports.length?data.supports.slice(0,3).map(money).join(' · '):'—'}/><Metric label="阻力" value={data.resistances.length?data.resistances.slice(0,3).map(money).join(' · '):'—'}/><Metric label="动量 / 波动阶段" value={`${data.momentum} / ${data.volatilityPhase}`}/></div>
      {(error||data.warnings.length>0)&&<div className="warning-row"><AlertTriangle/>{[error,...data.warnings].filter(Boolean).join('；')}</div>}
    </>}
  </section>
}
...
function NewsPanel({ data, error }: { data: NewsResponse|null; error: string }) {
  ...
  return <section className="panel wide-panel news-panel"><div className="section-title"><span><Newspaper size={17}/>消息面新闻</span><div className="news-summary"><b className={score>0?'up':score<0?'down':''}>{signed(score,1)} / 15</b><span>{data?.analysis.windowHours??48}h 窗口</span><span className={`module-status ${data?.analysis.status??'unavailable'}`}>{data?.analysis.status==='fresh'?'新鲜':data?.analysis.status==='partial'?'部分来源异常':'暂不可用'}</span></div></div>
    {error&&!data?<div className="module-empty"><AlertTriangle/>{error}，技术分析仍可正常使用</div>:!data?<div className="module-empty"><LoaderCircle className="spin"/>正在载入最近消息面…</div>:<>
      <div className="news-grid">{data.items.length?data.items.map(item=><article className="news-card" key={item.id}><div className="news-meta"><span className={`importance i${Math.round(item.importance)}`}>{item.importanceLabel}</span><span className={item.sentiment>0?'positive':item.sentiment<0?'negative':'neutral'}>{item.sentimentLabel}</span><span>{item.category}</span></div><h3>{item.url?<a href={item.url} target="_blank" rel="noopener noreferrer">{item.title}<ExternalLink/></a>:item.title}</h3>{item.summary&&<p>{item.summary}</p>}<div className="news-foot"><span>{item.source} · {time(item.publishedAt)}</span><span>相关度 {Math.round(item.relevance*100)}%</span></div>{item.reason&&<div className="news-reason">权重依据：{item.reason}</div>}</article>):<div className="module-empty">最近 {data.analysis.windowHours} 小时暂无合资格新闻</div>}</div>
      ...
      <p className="fineprint">消息分由来源、重要程度、BTC 相关度、方向和时效衰减综合计算，单独限制在 ±15；新闻标题不应作为独立入场依据。</p>
    </>}
  </section>
}
...
export default function App() {
  ...
  const load=useCallback(async()=>{ setLoading(true); const results=await Promise.allSettled([api.snapshot(),api.advice(),api.history(),api.settings(),api.technicalSummary(),api.news()]); if(results[0].status==='fulfilled') setSnapshot(results[0].value); if(results[1].status==='fulfilled') setAdvice(results[1].value); if(results[2].status==='fulfilled') setHistory(results[2].value); if(results[3].status==='fulfilled') setSettings(results[3].value); if(results[4].status==='fulfilled'){setTechnical(results[4].value);setTechnicalError('')}else setTechnicalError('技术共振摘要暂时无法获取'); if(results[5].status==='fulfilled'){setNews(results[5].value);setNewsError('')}else setNewsError('消息面数据暂时无法获取'); const failed=results.slice(0,4).filter(r=>r.status==='rejected').length; setError(failed>=3?'无法连接本地分析服务，请先启动后端。':''); setLoading(false) },[])
