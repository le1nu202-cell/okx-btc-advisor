...
export const workbenchApi = {
  ...
  deleteLog: (source: 'live' | 'replay', id: string) => request<{ deleted: boolean; deletedCount: number; source: string; id: string }>(`/api/trade-logs/${encodeURIComponent(id)}?source=${source}`, 'DELETE'),
  bulkDeleteLogs: (source: 'live' | 'replay', ids: string[]) => request<{ deletedCount: number; source: string }>('/api/trade-logs/bulk-delete', 'POST', { source, ids }),
  clearLogs: (source: 'live' | 'replay') => request<{ deletedCount: number; source: string }>(`/api/trade-logs?source=${source}&confirmation=DELETE`, 'DELETE'),
  exportLogsUrl: (source: 'live' | 'replay', format: 'csv' | 'json') => `/api/trade-logs/export?source=${source}&format=${format}`,
