...
- **估算强平价**：计划与实际分别按全仓可用权益、实际剩余 BTC、公开维持保证金档位、费用和可选资金费估算；公开参数缺失时不会猜测，可切换为人工维持保证金率。AUTO 模式只采用公开档位并忽略表单中残留的手工 MMR/固定额；标记价超过 30 秒时仍可保留不依赖现价的估算价，但距离和危险等级明确显示不可用。页面始终注明本工具不读取 OKX 账户，结果以 OKX 实际显示为准。公式与假设见 [docs/LIQUIDATION_ESTIMATE.md](./docs/LIQUIDATION_ESTIMATE.md)。
