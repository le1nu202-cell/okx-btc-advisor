...
双击后会通过 `launch.ps1` 启动本地服务并打开 [http://127.0.0.1:8765](http://127.0.0.1:8765)。启动器只复用身份和版本都匹配的本应用服务；升级后若检测到由本项目受信 Python 运行时启动的旧版本，会核对本机端口与完整 uvicorn 命令后自动关闭旧进程并启动当前版本，不会按健康接口名称盲目终止其他程序。冷启动时会校验或安装项目依赖并重新生成生产构建，失败时会显示说明并把独立诊断日志写入 `logs/`。连续双击会复用同一个服务。
