...
def analyze(c1:list[Candle],c4:list[Candle],funding_rows:list[tuple[int,float]]|None=None,oi:tuple[int,float]|None=None,custom=False,now_ms=None,news_analysis:dict|None=None)->SignalAdvice:
    ...
    funding_rows=[x for x in funding_rows if decision_at-90*86400_000<=x[0]<=decision_at]
    ...
    if regime==MarketRegime.TREND:
        ...
        contributions=[IndicatorContribution(name="布林带回归",score=base*.7,value=float((c[-1]-mid[-1])/(upper[-1]-lower[-1])),explanation="价格重新进入区间" if base else "尚无边界回归确认"),IndicatorContribution(name="RSI反转",score=base*.3,value=float(rr[-1]),explanation="RSI与区间边界共同确认" if base else "等待RSI反转"),IndicatorContribution(name="Stoch RSI确认",score=stoch_score,value=float(sk[-1]),explanation="动量交叉确认" if stoch_score else "尚未确认"),IndicatorContribution(name="MFI资金流",score=mfi_score,value=float(mf[-1]),explanation="量价资金流确认" if mfi_score else "尚未确认")]
        strategy="range"
    else:
        return SignalAdvice(strategy="regime",candle_close_at=ts,regime=regime,action=AdviceAction.WAIT,direction_score=0,technical_score=0,news_score=0,news_analysis=news_analysis,confidence=35,contributions=[IndicatorContribution(name="4H ADX",score=0,value=rv.get("adx"),explanation="趋势与震荡阈值之间")],invalidation="等待4H市场状态明确",explanation="当前处于过渡状态，不强行选择方向；消息面仅供观察。",data_quality=quality,config_version="custom-v2-unvalidated" if custom else "research-v2-unvalidated")
    technical_score=max(-85,min(85,score))
